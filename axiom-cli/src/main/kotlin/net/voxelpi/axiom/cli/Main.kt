...
private fun printCompilationStackTrace(exception: Throwable, terminal: Terminal, depth: Int = 0) {
    ...
    when (exception) {
        is ParseException -> {
            ...
            when (source) {
                is SourceLink.CompilationUnitSlice -> {
                    terminal.println(Text("$prefix Failed to parse ${sourceTextStyle("\"${source.text}\"")} at ${sourceRefStyle("${source.line + 1},${source.column + 1}")} of unit ${sourceUnitStyle("\"${source.unit.id}\"")}: ${errorStyle(exception.message ?: "")}"), true)
                }
        ...
        is SourceCompilationException -> {
            ...
            when (source) {
                is SourceLink.CompilationUnitSlice -> {
                    terminal.println(Text("$prefix Failed to compile ${sourceTextStyle("\"${source.text}\"")} at ${sourceRefStyle("${source.line + 1},${source.column + 1}")} of unit ${sourceUnitStyle("\"${source.unit.id}\"")}: ${errorStyle(exception.message ?: "")}"), true)
                }
                is SourceLink.Generated -> {
