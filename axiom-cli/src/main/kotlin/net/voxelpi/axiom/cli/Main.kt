            ...
            if (true) {
                rawFile.writeText(
                    program.instructions.joinToString("\n") { instruction ->
                        var text = instruction.toString().padEnd(48)
                        val source = instruction.meta[Assembler.SOURCE_INSTRUCTION_META_KEY] as? SourceLink
                        if (source != null) {
                            when (source) {
                                is SourceLink.CompilationUnitSlice -> {
                                    text += " # \"${source.text.trim()}\" from unit \"${source.unit.id}\" at ${source.line}:${source.column}"
                                }
                                is SourceLink.Generated -> {
                                    text += " # \"${source.text.trim()}\" generated by \"${source.generator}\""
                                }
                            }
                        }
                        text
