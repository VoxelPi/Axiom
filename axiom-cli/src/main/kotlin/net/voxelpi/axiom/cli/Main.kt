    ...
    class Assemble : Subcommand("assemble", "Assembles a program.") {
        ...
        override fun execute() {
            ...
            val program = assembler.assemble(inputFilePath, architecture).getOrElse { exception ->
                ...
                printCompilationStackTrace(exception)
                exitProcess(1)
            }
...
private fun printCompilationStackTrace(exception: Throwable, depth: Int = 0) {
    val prefix = "${"    ".repeat(depth)}  └ "
    when (exception) {
        is ParseException -> {
            System.err.println("$prefix ${exception.message}")
        }
        is SourceCompilationException -> {
            val source = exception.source
            when (source) {
                is SourceLink.CompilationUnitSlice -> {
                    System.err.println("$prefix Failed to compile \"${source.text}\" at ${source.line + 1},${source.column + 1} of unit \"${source.unit.id}\": ${exception.message}")
                }
                is SourceLink.Generated -> {
                    System.err.println("$prefix Failed to compile \"${source.text}\" generated by \"${source.generator}\": ${exception.message}")
                }
            }
            exception.cause?.let { printCompilationStackTrace(it, depth + 1) }
        }
        is CompilationException -> {
            System.err.println("$prefix Failed to compile code: ${exception.message}")
            exception.cause?.let { printCompilationStackTrace(it) }
        }
        else -> {
            throw exception
        }
